...
class BookmarksViewModelSpec: QuickSpec {
    ...
    override func spec() {
        ...
        """,
            ...
            viewModel = BookmarksViewModel.shared
        ...
        describe("Get UserResource by Group") {
            context("Succeeded") {
                it("UserResource is Returned") {
                    viewModel.bookmarksFeed(group: group)
                    
                    if case let .Success(resources) = viewModel.resourcesUiState.state {
                        expect { resources.map({ $0.resource.id }) }.to(equal(group.bookmarks.map({ $0.idResource }).reversed()))
                    }
                }
            }
        }
